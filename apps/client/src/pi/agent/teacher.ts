...
function teacherPrompt(
  ...
  sessionRule += [
    ...
    "For each image, describe its subject, action or knowledge relationships, composition, palette, and a suitable 2D visual style. Choose watercolor or colored-pencil picture-book art for warm story scenes, flat vector or hand-drawn diagrams for concepts, and comics only when panels or expressive action genuinely help. Avoid 3D rendering. A few short Chinese labels, signs, or speech bubbles are allowed when they improve the image; specify their exact wording and avoid text-heavy layouts. Put precise titles, formulas, and longer explanations in the app's text. Always provide meaningful alt text and never ask for watermarks or logos.",
  ].join(" ");
  return [
