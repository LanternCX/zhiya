...
桌面工作区采用全高侧栏，展开宽度为 220px，可收为 72px 图标栏。内容滚动时导航保持稳定。侧栏使用独立的次级背景和 1px 分隔线，不做成带圆角的浮动卡片。导航行高为 40px，间距紧凑；选中项使用柔和品牌底色、较强字重和左侧局部指示线，不使用硬偏移阴影。
